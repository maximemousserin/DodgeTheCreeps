...
public partial class Game : Node2D, IGame
{
  ...
  public const string MOB_SCENE_PATH = "res://src/mob/Mob.tscn";
  ...
  [Node] public ITimer MobTimer { get; set; } = default!;
  [Node] public ITimer ScoreTimer { get; set; } = default!;
  [Node] public ITimer StartTimer { get; set; } = default!;
  [Node] public IMarker2D StartPosition { get; set; } = default!;
  [Node] public IPath2D MobPath { get; set; } = default!;
  [Node] public Node2D Mobs { get; set; } = default!;
  ...
  public void OnResolved()
  {
    ...
    GameBinding
      .Handle(
        ...
        {
          // Start player at a default position
          Player.Start(StartPosition.Position);
          // Delete nodes in group mobs
      ...
      .Handle((in GameLogic.Output.SpawnMob _) => HandleSpawnMob());
  ...
  {
    var mobScene = GD.Load<PackedScene>(MOB_SCENE_PATH);
    var mob = mobScene.Instantiate<Mob>();

    var mobSpawnLocation = MobPath.GetNode<PathFollow2D>("MobSpawnLocation");
  ...
  private void OnMobTimerTimeout() => GameLogic.Input(new GameLogic.Input.MobTimerTimeout());
  private void OnScoreTimerTimeout() => GameLogic.Input(new GameLogic.Input.ScoreTimerTimeout());
  private void OnStartTimerTimeout() => GameLogic.Input(new GameLogic.Input.StartTimerTimeout(MobTimer, ScoreTimer));
}
